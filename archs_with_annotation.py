...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        super().__init__()
        img_size = to_2tuple(img_size) #patch_embed3:64
        patch_size = to_2tuple(patch_size)##patch_embed3:(3,3)

        self.img_size = img_size
        self.patch_size = patch_size
        self.H, self.W = img_size[0] // patch_size[0], img_size[1] // patch_size[1]
        self.num_patches = self.H * self.W
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=stride, ##patch_embed3:stride=2
    ...
    def forward(self, x): #t3.shape=[4,128,32,32]
        x = self.proj(x) #embed_dim[1]=160,so the output x.shape=[4,160,16,16]
        _, _, H, W = x.shape
        x = x.flatten(2).transpose(1, 2) #x.shape=[4,16*16,160]
        x = self.norm(x) #LayerNorm on normalized_shape 160
...
class UNext(nn.Module):
    ...
    def forward(self, x):
        ...
        t1 = out  #t1.shape=[4,16,128,128]
        ### Stage 2
        out = F.relu(F.max_pool2d(self.ebn2(self.encoder2(out)),2,2))
        t2 = out  #t2.shape=[4,32,64,64]
        ### Stage 3
        out = F.relu(F.max_pool2d(self.ebn3(self.encoder3(out)),2,2))
        t3 = out  #t3.shape=[4,128,32,32]

        ### Tokenized MLP Stage
        ### Stage 4

        out,H,W = self.patch_embed3(out) #out.shape=[4,256,160] H,W=16,16
        for i, blk in enumerate(self.block1):
            out = blk(out, H, W) #out.shape=[4,256,160]
        out = self.norm3(out)
        out = out.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        t4 = out #t4.shape = [4,160,16,16]
        ...
        _,_,H,W = out.shape
        out = out.flatten(2).transpose(1,2)#out.shape=[4,256,160]
        ...
        out = self.dnorm3(out)
        out = out.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()#out.shape = [4,160,16,16]
        out = F.relu(F.interpolate(self.dbn2(self.decoder2(out)),scale_factor=(2,2),mode ='bilinear'))
        out = torch.add(out,t3)
        _,_,H,W = out.shape
        out = out.flatten(2).transpose(1,2) #out.shape=[4,1024,128]
        
        for i, blk in enumerate(self.dblock2):
            out = blk(out, H, W)

        out = self.dnorm4(out)
        out = out.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous() #out.shape=[4,128,32,32]

        out = F.relu(F.interpolate(self.dbn3(self.decoder3(out)),scale_factor=(2,2),mode ='bilinear'))
        out = torch.add(out,t2) #[4,32,64,64]
        out = F.relu(F.interpolate(self.dbn4(self.decoder4(out)),scale_factor=(2,2),mode ='bilinear'))
        out = torch.add(out,t1) #[4,16,128,128]
        out = F.relu(F.interpolate(self.decoder5(out),scale_factor=(2,2),mode ='bilinear')) #out.shape=[4,16,256,256]
...
if __name__ == '__main__':
    ...
    net = UNext(num_classes = 10, input_channels=3, deep_supervision=False,img_size=256, patch_size=16, in_chans=3,  embed_dims=[ 128, 160, 256],
                 num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[1, 1, 1], sr_ratios=[8, 4, 2, 1])
    output = net(input)
    print(output.shape) #[4,10,256,256]
